...
3. ~~**Dashboard social features**~~ — ✅ v1 shipped 2026-07-04: "Channel Leaderboard" panel in the session dashboard — top tracks, top artists, top requesters (musicHistory playCounts) and most-dragged tracks (new dragStats instrumentation on queue reorders). Deferred: per-user attribution of web actions (needs auth inside the session dashboard) and search-log stats.
