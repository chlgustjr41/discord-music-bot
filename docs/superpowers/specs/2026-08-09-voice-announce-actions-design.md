...
| Trust in the directive | The plugin **validates the URL's origin against the configured web-app origin** before opening. See Security. |
...
`VoiceResult` gains `client`. `voice.ts` renders `detail` exactly as today, then walks `client` and, for each `open_url` directive whose origin passes validation, calls `streamDeck.system.openUrl`. Unknown directive types are ignored — forward compatibility, so an older plugin against a newer server degrades rather than breaking.
...
The plugin lets users override `apiUrl` in advanced settings, so the responding server is not guaranteed to be the real one. A hostile or misconfigured endpoint could otherwise drive the browser to a phishing page — and this codebase has already had one phishing finding (the sign-in state-relay, fixed by same-address binding). Therefore:

- The plugin validates each `open_url` directive's **origin against the configured web-app origin** and drops any that do not match. A trusted-server assumption is not worth making when the check is this cheap.
- Unknown directive types are ignored rather than dispatched generically. The directive vocabulary is closed, exactly like the action vocabulary.
...
| Directive origin fails validation | Plugin drops it silently and still renders `detail` |
...
- **Plugin:** a matching-origin directive opens the URL; a foreign-origin one does **not**; an unknown type is ignored; `detail` still renders in every case.
